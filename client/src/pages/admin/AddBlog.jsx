...
const AddBlog = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50  text-gray-600 h-full overflow-scroll'>
      ...
      <div className='bg-white w-full shadow rounded max-w-3xl p-4 md:p-10 sm:m-10'>
         <p>Uplod thumbnail</p>
         <label htmlFor="image">
         ...
         <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-1 relative'>
            <div ref={editorRef}>
            </div>
            {loading && (<div className='absolute right-0 top-0 bottom-0 left-0 flex justify-center items-center bg-black/10 mt-2'>
                <div className='w-8 h-8 rounded-full border-2 border-t-white animate-spin'></div>
            </div>)}
            <button disabled={loading} className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded cursor-pointer hover:underline' type='button' onClick={generateContent}>Generate with AI</button>
         </div>
